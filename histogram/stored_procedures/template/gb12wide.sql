...
-- change moodle_database to your database 
-- change moodle__database_user to your moodle user
-- change localhost to your moodle db host
-- change mdl_  to your moodle prefix 
use moodle_database;
drop procedure gb12wide;
delimiter //
CREATE DEFINER=`root`@`localhost` PROCEDURE `gb12wide`( IN gidinput int )
BEGIN
set @gid = gidinput;
set @scoremax = (select mdl_grade_items.grademax from mdl_grade_items where mdl_grade_items.id = @gid);
set @cid = (select mdl_grade_items.courseid from mdl_grade_items where mdl_grade_items.id = @gid);
set @scoremin = 0;
set @scorebucket = @scoremax / 10;
...
//
delimiter ;
grant execute on procedure  moodle_database.gb12wide to 'moodle__database_user'@'localhost';
flush privileges;
